...
public class CodeGeneratorNode {
    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            Long appId = 0L;
            // 先使用固定的 userMessageId (后续再整合到业务中)
            Long userMessageId = 0L;
            // 版本目录
            String versionDir = "v1";
            // 调用流式代码生成
            Flux<String> codeStream = codeGeneratorFacade.generateAndSaveCodeStream(userMessage, generationType, generationModel, appId, userMessageId, versionDir);
            // 同步等待流式输出完成
            codeStream.blockLast(Duration.ofMinutes(10)); // 最多等待 10 分钟
            // 根据类型设置生成目录
            String generatedCodeDir = String.format("%s/%s_%s_%s", AppConstant.CODE_OUTPUT_ROOT_DIR, generationType.getValue(), appId, versionDir);
